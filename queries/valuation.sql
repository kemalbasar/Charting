SELECT B.COMPANY, A.MATERIAL, B.WAREHOUSE,SUM(B.SKQUANTITY * (1 - 2 * B.QPOSTWAY)) AS QUANTITY,
	 A.MATTYPE, A.MATGRP2 AS MATGRP, T1.MATACGRP, T1.STEXT AS MALCINSI,
	 A.CATEGORY,A.ACIKLAMA,A.GRUBU,A.PARCA, CASE WHEN (SELECT TOP 1 IASVERITEM.PRICE AS PRICE
	...
	WHERE IASVERHEAD.CLIENT = '00'
		...
		AND IASVERITEM.COMPANY = B.COMPANY  ORDER BY IASVERITEM.CREATEDAT DESC) IS NOT NULL THEN (SELECT TOP 1 CONVERT(MONEY,(CASE WHEN IASVERHEAD.CURRENCY = 'TL' THEN CONVERT(MONEY, IASVERITEM.PRICE / IASVERITEM.PUNIT) ELSE (CONVERT(MONEY,IASVERITEM.PRICE * IB.EXCHRATEPUR / IASVERITEM.PUNIT)) END)/ID.EXCHRATEPUR) AS PRICE FROM IASVERITEM , IASMATBASIC , IASVERHEAD
	...
	LEFT OUTER JOIN IASBAS012 ID ON (ID.CURDATE =CONVERT(DATE,GETDATE())
		AND ID.CURRENCY ='EUR'
		AND ID.COMPANY = VLFMALFIYAT.COMPANY) WHERE VLFMALFIYAT.MATERIAL=A.MATERIAL
		AND VLFMALFIYAT.COMPANY=A.COMPANY ORDER BY VLFMALFIYAT.CREATEDAT DESC) END AS PRICE FROM  IASMATBASIC A ,IASINVITEM B
	LEFT OUTER JOIN (SELECT IASMATFMS.COMPANY, IASMATFMS.PLANT, IASMATFMS.MATACGRP, IASMATFMS.MATERIAL, IASBAS034.STEXT, IASMATFMS.FINCURRENCY FROM IASMATFMS , IASBAS034 , IASMATBASIC WHERE IASMATBASIC.CLIENT = IASMATFMS.CLIENT
		...
		AND A.ISDELETE = 0
                AND A.MATTYPE NOT IN ('IBKM','BSRF')
		AND B.WAREHOUSE  NOT IN ('BKM','600','GTP','700','800')
		AND (B.WAREHOUSE != (CASE SUBSTRING(A.MATERIAL,1,3) WHEN 'OTK' THEN  '100' ELSE  'BKM'  END))

		GROUP BY B.COMPANY, B.PLANT, B.WAREHOUSE,B.SPECIALSTOCK, A.MATERIAL, A.COMPANY,B.ISVARIANT, B.VARIANTKEY, B.VOPTIONS ,A.MATTYPE,A.MATGRP2 ,T1.MATACGRP,
		T1.STEXT,A.ACIKLAMA,A.GRUBU,A.PARCA,A.CATEGORY
