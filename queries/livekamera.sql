WITH AYK AS (
  SELECT TOP 1 A.CONFIRMATION, A.MATERIAL ,(A.OK - B.OK) AS QUANTITY ,  ((A.NOTOKGORSEL - B.NOTOKGORSEL) + (A.NOTOKOLCUSEL - B.NOTOKOLCUSEL)) AS NOTOK,(A.NOTOKGORSEL - B.NOTOKGORSEL) AS NOTOKGORSEL , (A.NOTOKOLCUSEL - B.NOTOKOLCUSEL) AS NOTOKOLCUSEL,
	 ...
	 C.LAST_THIRTY
	FROM [dbo].[XYZ]  A
	LEFT JOIN (SELECT TOP 1 MATERIAL,MIN(OK) AS OK , MIN(NOTOKGORSEL) AS NOTOKGORSEL , MIN(NOTOKOLCUSEL) AS NOTOKOLCUSEL , MIN(CURDATETIME) AS STARTTIME FROM [dbo].[XYZ]  WHERE (CURDATETIME) >= (CAST(CAST(CAST(GETDATE() AS DATE) AS VARCHAR) AS DATETIME) + ' 07:00:00')  GROUP BY MATERIAL ORDER BY MIN(CURDATETIME) DESC) B ON A.MATERIAL = B.MATERIAL

	LEFT JOIN ( SELECT MATERIAL,CONFIRMATION,DATEADD(MINUTE, -30, GETDATE())  AS STARTTIME,COUNT(CONFIRMATION) AS LAST_THIRTY FROM [dbo].[XYZ]  WHERE CURDATETIME >= DATEADD(MINUTE, -30, GETDATE()) GROUP BY MATERIAL, CONFIRMATION) C  ON A.MATERIAL = C.MATERIAL
		AND A.CONFIRMATION = C.CONFIRMATION
...
SELECT DISTINCT TOP 1 'XYZ' AS MACHINE,(CAST(CAST(CAST(GETDATE() AS DATE) AS VARCHAR) AS DATETIME) + ' 07:00:00') AS DAYSTART,
       ...
       WHERE P.POTYPE = 'N2' AND AYK.CONFIRMATION IS NOT NULL AND P.PRDORDER IS NOT NULL
