WITH AYK AS (
  SELECT TOP 1 A.CONFIRMATION , A.MATERIAL ,A.OK AS QUANTITY , (A.NOTOKGORSEL+A.NOTOKOLCUSEL) AS NOTOK,A.NOTOKGORSEL,A.NOTOKOLCUSEL,
	...
	FROM [dbo].[XYZ]  A
	LEFT JOIN (SELECT TOP 1 MATERIAL,CONFIRMATION,MIN(CURDATETIME) AS STARTTIME FROM [dbo].[XYZ]  WHERE (CURDATETIME) >= (CAST(CAST(CAST(GETDATE() AS DATE) AS VARCHAR) AS DATETIME) + ' 07:00:00')  GROUP BY MATERIAL,CONFIRMATION ) B ON A.MATERIAL = B.MATERIAL
...
SELECT DISTINCT TOP 1 'XYZ' AS MACHINE,(CAST(CAST(CAST(GETDATE() AS DATE) AS VARCHAR) AS DATETIME) + ' 07:00:00') AS DAYSTART,
       ...
       (CAST((SUM(AYK.QUANTITY) /(AYK.TOTALTIME - AYK.FAILURETIME )) AS DECIMAL) / 60) AS SANIYE_DENETLENEN,
