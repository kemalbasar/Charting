WITH ASD AS (SELECT B.COMPANY, A.MATERIAL, B.WAREHOUSE,SUM(B.SKQUANTITY * (1 - 2 * B.QPOSTWAY)) AS QUANTITY,
                A.MATTYPE, A.MATGRP2 AS MATGRP, T1.MATACGRP, T1.STEXT AS MALCINSI,A.CATEGORY,
                SUM(B.SKQUANTITY * (1 - 2 * B.QPOSTWAY))*CASE WHEN (SELECT TOP 1 IASVERITEM.PRICE AS PRICE 
                ...
                LEFT OUTER JOIN IASBAS012 ID ON (ID.CURDATE =CONVERT(DATE,GETDATE()) 
                               ...
                               AND VLFMALFIYAT.COMPANY=A.COMPANY ORDER BY VLFMALFIYAT.CREATEDAT DESC) END AS TOT_VALUE                 
...
SELECT MATTYPE,MALCINSI,SUM(ASD.QUANTITY) AS QUANTITY, SUM(TOT_VALUE) AS VALUE FROM ASD GROUP BY MATTYPE,MALCINSI
