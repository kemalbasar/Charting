...
def formatted_weeks_first():
    ...
    formatted_weeks.insert(0, 'OPERATION')
...
def update_graph_method():
    ...
    def pivotting_table():
        ...
        pivot_columns = df_sade.columns.difference(
            ['ANAMAMUL', 'MATERIAL','OPERATION', 'COSTCENTER', 'WORKCENTER', 'SURE_HESAPLAMA_KODU', 'MACHINE', 'LABOUR', 'SETUP',
             'BASEQUAN'])
        ...
        pivoted_df = df.melt(
            id_vars=['ANAMAMUL', 'MATERIAL', 'COSTCENTER', 'WORKCENTER', 'SURE_HESAPLAMA_KODU',
                     'MACHINE', 'LABOUR', 'SETUP','OPERATION',
    ...
    df_grouped = df_prepared.groupby(
        ['MATERIAL', 'COSTCENTER','OPERATION', 'WORKCENTER', 'SURE_HESAPLAMA_KODU', 'MACHINE', 'LABOUR', 'SETUP', 'BASEQUAN',
    ...
    pivot_result = df_grouped.pivot(
        index=['ANAMAMUL', 'MATERIAL','OPERATION', 'COSTCENTER', 'WORKCENTER', 'SURE_HESAPLAMA_KODU', 'MACHINE', 'LABOUR', 'SETUP',
               'BASEQUAN'],
...
create_table_from_df(pivot_result, "VLFPRESWORKHOURS")

insert_data_from_df(pivot_result, 'VLFPRESWORKHOURS')