...
def update_graph(selected_section):
    ...
    def pivotting_table():
        ...
        # Pivot the data
        pivoted_df = df.melt(
            id_vars=['MATERIAL', 'COSTCENTER', 'WORKCENTER', 'SURE_HESAPLAMA_KODU', 'MACHINE', 'LABOUR', 'SETUP',
    ...
    df_grouped = df_prepared.groupby(
        ['MATERIAL', 'COSTCENTER', 'WORKCENTER', 'SURE_HESAPLAMA_KODU', 'MACHINE', 'LABOUR', 'SETUP', 'BASEQUAN',
         'current_week']).agg({'value_min': 'sum'}).reset_index()

    # Son olarak, pivot işlemini gerçekleştirin
    pivot_result = df_grouped.pivot(
        index=['MATERIAL', 'COSTCENTER', 'WORKCENTER', 'SURE_HESAPLAMA_KODU', 'MACHINE', 'LABOUR', 'SETUP', 'BASEQUAN'],
    ...
    return [df_prepared.to_dict("records"),pivot_result.to_dict("records"),\
        pivot_result.to_dict('records'),formatted_columns,
    [{'if': {'column_id': column_id, 'filter_query': '{{{}}} = 0'.format(column_id)},'color': 'white'} for column_id in [col['id'] for col in formatted_columns_material]]]
