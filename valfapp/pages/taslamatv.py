...
layout7 = layout_for_tvs(costcenter='TASLAMA')
...
layout8 = html.Div([
    dbc.Col([
        ...
        dcc.Interval(id='row-switch-interval4', interval=30 * 1000, n_intervals=0)  # Switch every 30 seconds
...
layout = html.Div([
    dcc.Interval(id='layout-switch-interval4', interval=180*1000, n_intervals=0),  # Switch every 15 seconds
    html.Div(id='dynamic-layout-container4')
])

# Callback to switch between layouts
@app.callback(
    Output('dynamic-layout-container4', 'children'),
    Input('layout-switch-interval4', 'n_intervals')
)
...
@app.callback(
    Output('table_layout_8', 'data'),
    Output('table_layout_8', 'columns'),
    Output('table_layout_8', 'style_data_conditional'),
    Input('layout-switch-interval4', 'n_intervals'),
    Input('row-switch-interval4', 'n_intervals'),

)
def update_layout4_content(n,row_intervals):
    # Query data from the database
    data2 = ag.run_query(r"C:\Users\kereviz\Python Project\Charting\queries\livetaslamaplan.sql")
