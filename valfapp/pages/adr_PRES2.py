...
def return_sparks(graph1="fig_prod_forreportsttt", graph2="fig_scrap__forreportsttt", margin_left=0):
...
layout = dbc.Container([
    ...
    html.Div(id='refresh3_forreportsttt', style={'display': 'none'}),
    html.H2("PRESHANE2 Bölüm Raporu", style={
    ...
    dbc.Row([
        dbc.Col(children=[
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Col(id="my-output_forreportsttt1", width={"size": 2}, style={"margin-left": 0}),
                    dbc.Col([return_sparks(graph1="fig_prod_forreportsttt", graph2="fig_working_machine_forreportsttt",
                                           margin_left=0)],
                            width={"size": 2}),
                    dbc.Col(id="my-output_forreportsttt2", width={"size": 2}, style={"margin-left": 0}),
                    dbc.Col([return_sparks(graph1="fig_ppm_forreportsttt", graph2="fig_scrap__forreportsttt",
                                           margin_left=0)],
    ...
    dbc.Row([
        dbc.Col(children=[
            ...
            dbc.Row(
                dbc.Col([
                    html.Div(id='gann_forreportsttt')],
    ...
    dbc.Row([
        dbc.Col(children=[
            ...
            dbc.Row(
                dbc.Col([
                    html.Div(id='bubble_forreportsttt')],
                    className="d-flex justify-content-center", width=12)
                , className="g-0")],
            style={}, width={"size": 12}
        ), ]),
    dbc.Row([
        dbc.Col(children=[
            ...
            dbc.Row(
                dbc.Col([
                    html.Div(id='fig_scatscrap_forreportsttt')],
                    className="d-flex justify-content-center", width=12)
                , className="g-0")],
            style={}, width={"size": 12}
        ), ])
    ,
    html.Div(id="generated_1graph1data_for_reporttt")]
...
@app.callback(
    Output('location3_forreportsttt', 'href'),
    Input('refresh3_forreportsttt', 'children')
)
def page_refresh3_forreportsttt(n3):
    if n3:
        return "/prodeff"
    return no_update
...
@app.callback(
    [Output(component_id='my-output_forreportsttt1', component_property='children'),
     Output(component_id='my-output_forreportsttt2', component_property='children')],
...
@app.callback(
    Output(component_id='sunburst_forreportsttt', component_property='children'),
...
@app.callback(
    Output(component_id='bubble_forreportsttt', component_property='children'),
    Input(component_id='oeeelist2', component_property='data'))
def update_graph_bubble_forreportsttt(oeeelist2):
    graphwidth = 950
    oeeelist2 = pd.read_json(oeeelist2, orient='split')
    df, category_order = scatter_plot(df=oeeelist2.loc[oeeelist2["COSTCENTER"] == 'PRESHANE2'])
...
@app.callback(
    Output(component_id='gann_forreportsttt', component_property='children'),
    Input(component_id='oeeelist2', component_property='data')
)
def update_chart_gann_forreportsttt(oeeelist2):
    graphwidth = 900
    color_map = {"Uretim": "forestgreen", "Plansiz Durus": "red"
        , "Ariza Durusu": "Brown", "Planli Durus": "Coral"
        , "Kurulum": "Aqua"}
    oeeelist2 = pd.read_json(oeeelist2, orient='split')
    df = oeeelist2.loc[oeeelist2["COSTCENTER"] == 'PRESHANE2']
...
@app.callback(
    [Output(component_id='fig_prod_forreportsttt', component_property='figure'),
     Output(component_id='fig_scrap__forreportsttt', component_property='figure'),
     Output(component_id='fig_working_machine_forreportsttt', component_property='figure'),
     Output(component_id='fig_ppm_forreportsttt', component_property='figure')],
    Input(component_id='work-datees', component_property='data'),
    Input(component_id='oeeelist6', component_property='data')
)
def update_spark_line(dates, oeeelist6):
    ...
    fig_working_machine_forreportsttt = get_spark_line(
        data=working_machinesf(working_machines=df_working_machines))
    fig_ppm_forreportsttt = get_spark_line(
        data=generate_for_sparkline(data=onemonth_prdqty, proses='PRESHANE2', ppm=True))
    return [fig_prod_forreportsttt, fig_scrap__forreportsttt, fig_working_machine_forreportsttt, fig_ppm_forreportsttt]


@app.callback(
    [Output(component_id='fig_up1_forreportsttt', component_property='figure'),
     Output(component_id='fig_up2_forreportsttt', component_property='figure'),
     Output(component_id='fig_up3_forreportsttt', component_property='figure'),
     Output(component_id='fig_down1_forreportsttt', component_property='figure'),
     Output(component_id='fig_down2_forreportsttt', component_property='figure'),
     Output(component_id='fig_down3_forreportsttt', component_property='figure')
     ],
    Input(component_id='oeeelist1', component_property='data')
)
def update_ind_fig(oeeelist1):
    df = pd.read_json(oeeelist1, orient='split')
    df = df[df["COSTCENTER"] == 'PRESHANE2']
    fig_up1_forreportsttt = indicator_with_color(df_metrics=df)
    fig_up2_forreportsttt = indicator_with_color(df_metrics=df, order=1)
    fig_up3_forreportsttt = indicator_with_color(df_metrics=df, order=2)
...
@app.callback(
    Output(component_id='fig_scatscrap_forreportsttt', component_property='children'),
    Input(component_id='work-datees', component_property='data')
)
def create_scatterplot_for_scrapqty(dates):
    ...
    df_scrap = ag.run_query(query=r"EXEC VLFPRDSCRAPWITHPARAMS @WORKSTART=?, @WORKEND=?"
                            , params=(dates["workstart"], dates["workend"]))
    df_scrap = df_scrap[df_scrap["COSTCENTER"] == 'PRESHANE2']
    ...
    fig = px.histogram(data_frame=df_scrap.loc[df_scrap["COSTCENTER"] == 'PRESHANE2'],
                       x="WORKCENTER",
                       y="SCRAP",
                       color="STEXT",
                       color_discrete_map=color_map,
...
@app.callback(
    [Output("generated_1graph1data_for_reporttt", "children")],
...
def update_ind_fig(params, oeeelist1w, oeeelist3w, oeeelist7w):
    ...
    Args:
        ...
        option_slctd = 'PRESHANE2' (str): The selected cost center.
    ...
    def return_layout(report_type='wc'):
        list_of_figs, list_of_data, list_of_columns, list_of_styles = workcenters('PRESHANE2', report_type,
                                                                                  params,
